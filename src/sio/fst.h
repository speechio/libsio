...
struct Fst {
    ...
    Error BuildTokenTopology(const Tokenizer& tokenizer) {
        ...
        /* 1: Build Fst arcs */
        {
            ...
            this->start_state = 0;
            AddArc(this->start_state, this->start_state, tokenizer.blk, kFstEps);
            ...
            for (TokenId t = 0; t != tokenizer.Size(); t++) {
                ...
                AddArc(cur_state,         cur_state,         t,       kFstEps); // Self-loop
                AddArc(cur_state,         this->start_state, kFstEps, kFstEps); // Leaving
                cur_state++;
            }

            // 1c: "InputEnd" represents the end of input sequence (follows K2Fsa convention)
            this->final_state = cur_state;
            AddArc(this->start_state, this->final_state, kFstInputEnd, tokenizer.eos);
